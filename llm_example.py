...
chat_prompt_1 = PromptTemplate(
    ...
    template = """        
            ...
            ## 重要补充：
            - **如果检索到的相关信息（{context}）与任务无关或无助于提取设备属性，可以忽略该信息，仅基于工控系统文本（{text}）进行分析**。
            - 如果某些设备属性无法从工控系统文本中提取出来，则在输出格式中填充“不存在”。不要编造信息。
            - 如果工控系统文本中不存在任何可提取的单点设备，则输出“【不存在单点设备】”。
            - 如果工控系统文本中存在可提取的单点设备，则必须完整提取每一个设备，并按照规定的格式输出，不得遗漏。
...
chat_prompt_2 = PromptTemplate(
    input_variables = ["text","device","context","reasoning"],
    template = """
        这是**工控系统文本**，**所有分析都基于此文本**：{text}
        这是已经提取出的单点设备：{device}
        这是检索到的相关信息，**如果存在工控系统文本中不存在的内容请忽略**：{context}
        这是基于前一阶段提取单点设备的推理过程得到的相关知识：{reasoning}
        ...
        ## 处理流程:
        - 首先你需要根据基于前一阶段提取单点设备的推理过程得到的相关知识判断已经提取出的单点设备是否存在问题，如果有请修正，后面的分析都基于修正后的单点设备进行处理。
...
chat_prompt_3 = PromptTemplate(
    input_variables = ["text","action","context","reasoning"],
    template = """
    这是**工控系统文本**，**所有分析都基于此文本**：{text}
    这是已经提取出的系统业务拓扑：{action}
    这是检索到的相关信息，**如果存在工控系统文本中不存在的内容请忽略**：{context}
    这是基于前一阶段提取系统拓扑的推理过程得到的相关知识：{reasoning}
    ...
    ## 处理流程:
    - 首先你需要根据基于前一阶段提取单点设备的推理过程得到的相关知识判断已经提取出的业务拓扑是否存在问题，如果有请修正，后面的分析都基于修正后的单点设备进行处理。
